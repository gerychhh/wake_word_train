...
def generate_adversarial_texts(input_text: str, N: int, include_partial_phrase: float = 0, include_input_words: float = 0):
    ...
    """
    # ---------------------------------------------------------------------
    # Fallback for non-English / non-ASCII input (e.g. Cyrillic wake words).
    #
    # The phoneme-based adversarial generation below is English-centric
    # (CMUDICT/"pronouncing" + optional DeepPhonemizer). For wake words that
    # contain non-ASCII characters, we fall back to simple character-level
    # edits. This still produces useful "near miss" phrases for Piper TTS
    # without adding heavy dependencies.
    def _simple_word_variants(word: str, n: int = 40) -> list[str]:
        word = (word or "").strip()
        if not word:
            return []

        is_cyr = re.search(r"[\u0400-\u04FF]", word) is not None
        alphabet = (
    ...
    def _simple_phrase_variants(words: list[str], n_out: int) -> list[str]:
        words = [w for w in (words or []) if w]
        if not words:
            return []

        original = " ".join(words)
        per_word = [_simple_word_variants(w, n=30) for w in words]
        out: set[str] = set()

        max_tries = max(200, n_out * 200)
        for _ in range(max_tries):
            chosen = []
            for w, variants in zip(words, per_word):
                if variants and random.random() > include_input_words:
                    chosen.append(random.choice(variants))
                else:
                    chosen.append(w)

            # Ensure at least one token differs
            if chosen == words:
                idx = random.randrange(len(words))
                if per_word[idx]:
                    chosen[idx] = random.choice(per_word[idx])

            # Optionally return a partial phrase (multi-word wake phrases)
            if include_partial_phrase and len(chosen) > 1 and random.random() < include_partial_phrase:
                a = random.randrange(0, len(chosen))
                b = random.randrange(a + 1, len(chosen) + 1)
                chosen = chosen[a:b]

            phrase = " ".join(chosen).strip()
            if phrase and phrase != original:
                out.add(phrase)
            if len(out) >= n_out:
                break
    ...
    if any(ord(c) > 127 for c in input_text):
        return _simple_phrase_variants(input_text.split(), N)
    ...
    # Download phonemizer model for OOV words, if needed (English-only path).
    phonemizer = None
    if [] in input_text_phones:
        try:
            ...
            logging.warning(
                "DeepPhonemizer is not available (%s). Falling back to simple string adversarials.",
                e,
            )
            return _simple_phrase_variants(input_text.split(), N)

        resources_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "resources")
        phonemizer_mdl_path = os.path.join(resources_dir, "en_us_cmudict_forward.pt")
        os.makedirs(resources_dir, exist_ok=True)

        if not os.path.exists(phonemizer_mdl_path):
            logging.warning("Downloading DeepPhonemizer model (en_us_cmudict_forward.pt)...")
            try:
                ...
                logging.warning(
                    "Failed to download DeepPhonemizer model (%s). Falling back to simple string adversarials.",
                    e,
                )
                return _simple_phrase_variants(input_text.split(), N)
        ...
        try:
            ...
            logging.warning(
                "Failed to initialize DeepPhonemizer (%s). Falling back to simple string adversarials.",
                e,
            )
            return _simple_phrase_variants(input_text.split(), N)
    ...
    for phones, word in zip(input_text_phones, input_text.split()):
        if phones != []:
            ...
            try:
                ...
                logging.warning(
                    "DeepPhonemizer failed for '%s' (%s). Falling back to simple string adversarials.",
                    word,
                    e,
                )
                return _simple_phrase_variants(input_text.split(), N)
